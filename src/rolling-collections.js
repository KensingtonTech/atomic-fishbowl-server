...
class RollingCollectionManager {
  ...
  constructor(handler, collection, collectionId, rollingId) {
    ...
    this.workLoopWaitingToStart = false; // this is set to true when onWorkerExit restarts the workLoop after restartWorkLoopOnExit is true, and less than 60 seconds has elapsed since the start of the last loop.  During the time when the workLoop is waiting to start again, this will prevent future clients from starting additional workLoops.  It's for an edge case.
  ...
  addHttpClient(id, client) {
    ...
    if (this.runs > 0) {
      ...
      if (this.workInterval) {
        ...
        winston.debug('RollingCollectionManager: addHttpClient(): there is an existing workInterval running.  Not starting workLoop');
        return;
      }

      else if (this.workLoopWaitingToStart) {
        winston.debug('RollingCollectionManager: addHttpClient(): workLoopWaitingToStart is true.  Not starting the workLoop');
        return;
      }
        ...
        winston.debug(`RollingCollectionManager: addHttpClient(): workerProcess.length: ${this.workerProcess.length}`);

        if (this.workerProcess.length !== 0) {
          // there's already a worker process running.  When it exits, instruct it to restart the workLoop
          winston.debug('RollingCollectionManager: addHttpClient(): there is an existing workerProcess.  The workLoop will be restarted when it exits.  Setting restartWorkLoopOnExit to true');
          ...
          this.resumed = true;
          winston.debug('RollingCollectionManager: addHttpClient(): there is no existing workerProcess.  Starting workLoop');
  ...
  addSocketClient(socket) {
    ...
    if (this.runs > 0) {
      ...
      if (this.workInterval) {
        ...
        winston.debug('RollingCollectionManager: addSocketClient(): workLoopWaitingToStart is true.  Not starting the workLoop');
        return;
      }

      else if (!this.workInterval) {
        // there is no existing workInterval.  Better check for a running worker in case there's an old one still floating about
        winston.debug(`RollingCollectionManager: addSocketClient(): workerProcess.length: ${this.workerProcess.length}`);
  ...
  workLoop() {
    ...
    if (!this.monitoringCollection && this.resumed && (moment().unix() - this.lastRun < 60) ) {
      winston.debug(`RollingCollectionManager: workLoop(): Less than 60 seconds have elapsed since the last cycle run for rollingId ${this.rollingId}.  Skipping this cycle`);
      return;
    }
  ...
  onWorkerExit(code, signal, pid, tempName) {
    ...
    if (this.restartWorkLoopOnExit && this.workerProcess.length === 0) {
      ...
      let mom = moment().unix();
      let secondsSinceLastRun = mom - this.lastRun;
      if ( secondsSinceLastRun < 60) {
        ...
        winston.debug(`onWorkerExit(): delaying start of next workLoop cycle by ${60 - secondsSinceLastRun} seconds`);
  ...
  async onConnectionFromWorker(tempName, workerSocket) {
    ...
    if (this.monitoringCollection) {
    ...
    else if ( this.runs === 2 && !this.resumed && ( moment().unix() - this.lastRun > 60 ) ) {
    ...
    else if ( this.runs >= 2 && !this.resumed) {
    ...
    else if ( this.resumed && ( moment().unix() - this.lastRun < 60 ) ) {
      // The non-monitoring collection has resumed and less than 60 seconds have elapsed since the last run
      ...
      // shouldn't we just return here and wait for the next cycle to run? probably not, as the worker has already started by the time we get here.  This probably means we need to put a guard in somewhere else to prevent the worker from being launched if less than 60 seconds have elapsed since the last run.
