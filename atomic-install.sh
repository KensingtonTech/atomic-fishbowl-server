...
chroot $HOST /usr/bin/docker create --name $NAME --log-driver none --network afb-network --ip 172.31.255.243 --add-host afb-mongo:172.31.255.242 -v /etc/kentech:/etc/kentech:ro -v /var/kentech:/var/kentech:rw,z -v /var/run/systemd/journal/socket:/var/run/systemd/journal/socket -e SYSTEMD=1 $IMAGE >/dev/null
